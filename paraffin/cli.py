...
def worker(
    ...
    while True:
        ...
        result = subprocess.run(f"dvc repro -s {job['name']}", shell=True, capture_output=True)

        if result.returncode != 0:
            log.error(f"Failed to run job '{job['name']}'")
            log.error(result.stderr.decode())
            complete_job(job["id"], status="failed", lock={}, stdout=result.stdout.decode(), stderr=result.stderr.decode())
        else:
            # get the stage_lock
            fs = dvc.api.DVCFileSystem(url=None, rev=None)
            with fs.repo.lock:
                ...
                stage.save()
            stage_lock = to_single_stage_lockfile(stage, with_files=True)
            complete_job(job["id"], status="completed", lock=stage_lock, stdout=result.stdout.decode(), stderr=result.stderr.decode())
