...
from paraffin.db import complete_job, get_job, save_graph_to_db, find_cached_job
...
def worker(
    ...
    while True:
        job_obj = get_job(queues=queues, worker=name, machine=socket.gethostname(), experiment=experiment, job_name=job)
        ...
        with fs.repo.lock:
            ...
            reduced_lock = {k: v for k, v in stage_lock.items() if k in ["params", "deps", "cmd"]}
